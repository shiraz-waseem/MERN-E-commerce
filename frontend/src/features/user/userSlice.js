...
export const updateUserAsync = createAsyncThunk(
  "user/updateUser",
  async (update) => {
    const response = await updateUser(update); // this was name mistake (fixed it)
    // The value we return becomes the `fulfilled` action payload
    return response.data;
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateUserAsync.fulfilled, (state, action) => {
        state.status = "idle";
        // state.userOrders = action.payload;     // earlier there was loggedInUser variable in other slice
